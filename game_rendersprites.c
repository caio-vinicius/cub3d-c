...
/*   Updated: 2020/07/17 21:29:51 by caio             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		draw_sprite(t_cub *cub, t_sprite *sprite, t_data *data, t_img texture)
{
	...
	while (x < sprite->draw_end_x)
	{
		if (sprite->transform_y > 0 && sprite->draw_start_x >= 0 &&
				sprite->draw_start_x < (int)cub->width && sprite->transform_y < sprite->zbuffer[x])
		{
...
void	game_rendersprites(t_data *data, t_sprite sprite, t_cub *cub, t_player player, t_img texture)
{
	...
	while (i < sprite.amount)
	{
		sprite.x = (float)(sprite.positions[i].x) - (player.x / TILE_SIZE);
		sprite.y = (float)(sprite.positions[i].y) - (player.y / TILE_SIZE);
